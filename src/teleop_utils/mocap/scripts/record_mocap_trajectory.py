...
def main(rigid_body_names: list[str]):
    ...
    recording = False
    quit_flag = False
    results: dict[str, list[tuple[npt.NDArray[np.float64], float]]] = {rigid_body_name: [] for rigid_body_name in rigid_body_names}
    ...
    listener.start()
    last_timestamp: dict[str, float] = {rigid_body_name: 0 for rigid_body_name in rigid_body_names}
    while not quit_flag:
        if recording:
            for rigid_body_name in rigid_body_names:
                ...
                print(f"Object: {rigid_body_name}, Timestamp: {mocap_timestamp:.6f}, Position: {pose_xyz_wxyz}")
        time.sleep(1 / 120)
