    ...
    try: 
        for frame in container.decode(video=0):  # Decode video stream (index 0)
            ...
            for qrcode in qrcodes:
                ...
                frame_system_time = video_start_system_time + timedelta(seconds=frame.time)
                time_difference = frame_system_time.timestamp() - float(qrcode_data)
                time_differences_s.append(time_difference)
                print(f"Frame {frame_count}: QR Code Data: {qrcode_data}, frame system time: {frame_system_time.strftime('%H:%M:%S.%f')}, time difference: {time_difference}")
    except KeyboardInterrupt:
        print(f"Interrupted by user")

    print(f"Average time difference (camera timestamp - mocap timestamp): {np.mean(time_differences_s)}")
