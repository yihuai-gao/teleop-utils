...
import os
def interpolate_mocap_poses(poses: npt.NDArray[np.float64], timestamps: npt.NDArray[np.float64], new_timestamps: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    ...
    if new_timestamps[0] > timestamps[-1] or new_timestamps[-1] < timestamps[0]:
        print("New timestamps are out of range of the original timestamps. Skipping interpolation.")
    ...
    for i in range(poses.shape[1]):
        interpolated_poses[:, i] = interp.interp1d(timestamps, poses[:, i], kind='cubic', assume_sorted=True)(new_timestamps)
    return interpolated_poses
...
def main(mocap_file: str, video_file: str, time_difference: float):
    ...
    creation_date_time = datetime.strptime(video_stream.metadata["creation_time"], "%Y-%m-%dT%H:%M:%S.%fZ").replace(hour=0, minute=0, second=0, microsecond=0)
    # Example of timecode: 00:00:00:00
    video_start_system_time = timedelta(hours=int(video_stream.metadata["timecode"][:2]), minutes=int(video_stream.metadata["timecode"][3:5]), seconds=int(video_stream.metadata["timecode"][6:8]), microseconds=int(video_stream.metadata["timecode"][9:12]))
    ...
    video_frame_mocap_timestamps = video_frame_relative_timestamps + video_start_system_time.timestamp() - time_difference

    interpolated_results = {}
    for rigid_body_name, trajectory in data.items():
        original_poses = np.array([pose_xyz_wxyz for pose_xyz_wxyz, _ in trajectory])
        original_timestamps = np.array([mocap_timestamp for _, mocap_timestamp in trajectory])
        interpolated_poses = interpolate_mocap_poses(original_poses, original_timestamps, video_frame_mocap_timestamps)
        interpolated_results[rigid_body_name] = list(zip(interpolated_poses, video_frame_mocap_timestamps))
